...
namespace EDSDK_NET
{
    ...
    public class SDKHandler : IDisposable
    {
        ...
        {
            //get list of cameras
            Error = EdsGetCameraList(out IntPtr camlist);
            ...
            for (int i = 0; i < c; i++)
            {
                ...
                Error = EdsGetChildAtIndex(camlist, i, out IntPtr cptr);
        ...
        private uint Camera_SDKObjectEvent(uint inEvent, IntPtr inRef, IntPtr inContext)
        {
            ...
            switch (inEvent)
            {
                ...
                case ObjectEvent_DirItemRequestTransfer: DownloadImage(inRef, ImageSaveDirectory); break;
                case ObjectEvent_DirItemRequestTransferDT: break;
                case ObjectEvent_FolderUpdateItems: break;
                case ObjectEvent_VolumeAdded: break;
                case ObjectEvent_VolumeInfoChanged: break;
                case ObjectEvent_VolumeRemoved: break;
                case ObjectEvent_VolumeUpdateItems: break;
            }
        ...
        private uint Camera_SDKPropertyEvent(uint inEvent, uint inPropertyID, uint inParameter, IntPtr inContext)
        {
            ...
            switch (inPropertyID)
            {
                case PropID_AEBracket: break;
                case PropID_AEMode: break;
                case PropID_AEModeSelect: break;
                case PropID_AFMode: break;
        ...
        public Bitmap GetFileThumb(string filepath)
        {
            ...
            Error = EdsCreateFileStream(filepath, EdsFileCreateDisposition.OpenExisting, EdsAccess.Read, out IntPtr stream);
        ...
        public void SetSetting(uint PropID, uint Value)
        {
            if (MainCamera.Ref != IntPtr.Zero)
            {
                SendSDKCommand(delegate
                {
                    ...
                    Error = EdsGetPropertySize(MainCamera.Ref, PropID, 0, out EdsDataType proptype, out int propsize);
                    //set given property
            ...
            m_LVThread = STAThread.Create(delegate
            {
                try
                {
                    ...
                    while (IsLiveViewOn)
                    {
                        ...
                        //get pointer
                        Error = EdsGetPointer(stream, out IntPtr jpgPointer);
        ...
        public CameraFileEntry GetAllEntries()
        {
            ...
            Error = EdsGetChildCount(MainCamera.Ref, out int VolumeCount);
            ...
            for (int i = 0; i < VolumeCount; i++)
            {
                ...
                Error = EdsGetChildAtIndex(MainCamera.Ref, i, out IntPtr ChildPtr);
        ...
        private CameraFileEntry[] GetChildren(IntPtr ptr)
        {
            ...
            if (ChildCount > 0)
            {
                ...
                for (int i = 0; i < ChildCount; i++)
                {
                    ...
                    Error = EdsGetChildAtIndex(ptr, i, out IntPtr ChildPtr);
                    ...
                    if (!MainEntry[i].IsFolder)
                    {
                        ...
                        Error = EdsCreateMemoryStream(0, out IntPtr stream);
