...
namespace DxCapture
{
    public class VideoCaptureGraph : DSFilterGraphBase, IFileDestSupport
    {
        ...
        public string OutputFileName { get => m_sOutputFileName; set => Save(value, false); }

        public HRESULT Save() => Save(true);

        public HRESULT Save(bool bStart) => Save(m_sOutputFileName, bStart);

        public HRESULT Save(string sFileName) => Save(sFileName, false);
