...
namespace CanonCaptureFilter
{
       ...
       IAMBufferNegotiation
    {
        ...
        public ScreenCaptureStream(string name, BaseSourceFilter _filter) : base(name, _filter)
        {
        ...
        public override int CheckMediaType(AMMediaType pmt) => (m_Filter as ScreenCaptureFilter).CheckMediaType(pmt);

        public override int GetMediaType(int iPosition, ref AMMediaType pMediaType) => (m_Filter as ScreenCaptureFilter).GetMediaType(iPosition, ref pMediaType);
        ...
        public int SetFormat(AMMediaType pmt)
        {
            ...
            if (IsConnected)
            {
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    if (SUCCEEDED(hr))
                    {
                        hr = (HRESULT)(m_Filter as ScreenCaptureFilter).SetMediaType(newType);
                    }
                    else
                    {
                        m_mt.Set(oldType);
        ...
        {
            HRESULT hr = (HRESULT)(m_Filter as ScreenCaptureFilter).GetNumberOfCapabilities(out int iCount, out int iSize);
            if (hr.Failed) return hr;
        ...
        {
            HRESULT hr = (HRESULT)(m_Filter as ScreenCaptureFilter).GetStreamCaps(iIndex, out AMMediaType pmt, out VideoStreamConfigCaps _caps);
        ...
        public int GetLatency(out long prtLatency) => (m_Filter as ScreenCaptureFilter).GetLatency(out prtLatency);
