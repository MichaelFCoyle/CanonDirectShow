...
{
    [ComVisible(false)]
    public class VirtualCamStream : SourceStream, IAMStreamControl, IKsPropertySet, IAMPushSource, IAMLatency, IAMStreamConfig, IAMBufferNegotiation
    {
        ...
        public VirtualCamStream(string _name, BaseSourceFilter _filter) : base(_name, _filter)
        {
        ...
        public override int CheckMediaType(AMMediaType pmt) => (m_Filter as VirtualCamFilter).CheckMediaType(pmt);

        public override int GetMediaType(int iPosition, ref AMMediaType pMediaType) => (m_Filter as VirtualCamFilter).GetMediaType(iPosition, ref pMediaType);
        ...
        {
            HRESULT hr = (HRESULT)(m_Filter as VirtualCamFilter).GetNumberOfCapabilities(out int iCount, out int iSize);
            if (hr.Failed) return hr;
        ...
        {
            HRESULT hr = (HRESULT)(m_Filter as VirtualCamFilter).GetStreamCaps(iIndex, out AMMediaType pmt, out VideoStreamConfigCaps _caps);
        ...
        public int GetLatency(out long prtLatency) => (m_Filter as VirtualCamFilter).GetLatency(out prtLatency);

        #endregion
    }

    [ComVisible(true)]
    [Guid("3372F356-E2B2-42F9-8EEE-C7CA3A6005C7")]
    ...
    public class VirtualCamFilter : BaseSourceFilter, IAMFilterMiscFlags
    {
        ...
        public VirtualCamFilter() : base("CSharp Virtual Camera 2")
        {
        ...
        [DllImport("gdi32.dll")]
        private static extern int GetDIBits(IntPtr hdc, IntPtr hbmp, uint uStartScan, uint cScanLines, [Out] IntPtr lpvBits, ref BitmapInfo lpbmi, uint uUsage);
