...
namespace DirectShow.BaseClasses
{
    ...
    public abstract class BasePin : PropPageSupport, IPin, IQualityControl
    {
        ...
        public IQualityControl QualitySync => m_QualitySink != IntPtr.Zero ? (IQualityControl)Marshal.GetObjectForIUnknown(m_QualitySink) : null;

        public IMemAllocatorImpl Allocator => new IMemAllocatorImpl(m_pAllocator);
        ...
        public virtual int BreakConnect() => NOERROR;

        public virtual int CompleteConnect(ref IPinImpl pReceivePin) => NOERROR;
        ...
        public virtual int Run(long tStart) => NOERROR;

        public virtual int CheckConnect(ref IPinImpl _pin)
        {
            HRESULT hr = (HRESULT)_pin.QueryDirection(out PinDirection _direction);
            if (hr.Failed) return hr;
            if (_direction == m_Direction) return VFW_E_INVALID_DIRECTION;
            return NOERROR;
        }
        ...
        public virtual int GetMediaType(int iPosition, ref AMMediaType pMediaType) => E_UNEXPECTED;
        ...
        protected virtual int AttemptConnection(ref IPinImpl pReceivePin, AMMediaType pmt)
        {
            ...
            if (hr == NOERROR)
            {
                ...
                if (SUCCEEDED(hr))
                {
                    ...
                    Marshal.QueryInterface(_ptr, ref _guid, out IntPtr _this);
    ...
    public abstract class BaseInputPin : BasePin, IMemInputPin
    {
        ...
        public override int Notify(IntPtr pSelf, Quality q) => pSelf == IntPtr.Zero ? E_POINTER : (int)NOERROR;
    ...
    public abstract class BaseOutputPin : BasePin
    {
        ...
        public virtual int InitAllocator(out IntPtr ppAlloc) => CreateMemoryAllocator(out ppAlloc);
        ...
        public virtual int Deliver(ref IMediaSampleImpl _sample) => Deliver(_sample.UnknownPtr);

        public virtual int Deliver(IntPtr pSample) => m_pInputPin == IntPtr.Zero ? VFW_E_NOT_CONNECTED : InputPin.Receive(pSample);

        public virtual int DeliverEndOfStream() => m_ConnectedPin == IntPtr.Zero ? VFW_E_NOT_CONNECTED : Connected.EndOfStream();

        public virtual int DeliverBeginFlush() => m_ConnectedPin == IntPtr.Zero ? VFW_E_NOT_CONNECTED : Connected.BeginFlush();

        public virtual int DeliverEndFlush() => m_ConnectedPin == IntPtr.Zero ? VFW_E_NOT_CONNECTED : Connected.EndFlush();

        public virtual int DeliverNewSegment(long tStart, long tStop, double dRate) => m_ConnectedPin == IntPtr.Zero ? VFW_E_NOT_CONNECTED : Connected.NewSegment(tStart, tStop, dRate);
    ...
    public abstract class BaseFilter : PropPageSupport, IPersistStream, IBaseFilter
    {
        ...
        public virtual int IsDirty() => (m_bPersistDirty ? S_OK : S_FALSE);
    ...
    public class TransformOutputPin : BaseOutputPin, IMediaSeeking
    {
        ...
        public TransformOutputPin(string _name, TransformFilter _filter) : base(_name, _filter) { }
        ...
        public override int DecideBufferSize(ref IMemAllocatorImpl pAlloc, ref AllocatorProperties prop) => ((TransformFilter)m_Filter).DecideBufferSize(ref pAlloc, ref prop);
    ...
    public abstract class TransformFilter : BaseFilter
    {
        ...
        public TransformFilter(string _name) : base(_name) { }
    ...
    public class BasePropertyPage : Form, IPropertyPage
    {
        ...
        public BasePropertyPage() => Visible = false;

        public BasePropertyPage(string _title) : this() => this.Text = _title;
        ...
        public virtual int IsPageDirty() => m_bDirty ? COMHelper.S_OK : COMHelper.S_FALSE;

        public virtual int Apply()
        {
            if (m_bObjectSet == false) return HRESULT.E_UNEXPECTED;

            // Must have had a site set
            if (m_pPageSite == null) return HRESULT.E_UNEXPECTED;

            // Has anything changed
            if (m_bDirty == false) return HRESULT.NOERROR;
        ...
        public virtual int Help(string sHelpDir) => HRESULT.E_NOTIMPL;

        public virtual int TranslateAccelerator(IntPtr pMsg) => HRESULT.E_NOTIMPL;

        #endregion

        #region Virtual Methods

        public virtual HRESULT OnConnect(IntPtr pUnknown) => HRESULT.NOERROR;

        public virtual HRESULT OnDisconnect() => HRESULT.NOERROR;

        public virtual HRESULT OnActivate() => HRESULT.NOERROR;

        public virtual HRESULT OnDeactivate() => HRESULT.NOERROR;

        public virtual HRESULT OnApplyChanges() => HRESULT.NOERROR;
        ...
        public static bool ShowPropertyPages(IBaseFilter _filter, IntPtr _hwnd)
        {
            try
            {
                ...
                if (pProp != null)
                {
                    if (COMHelper.SUCCEEDED(pProp.GetPages(out DsCAUUID pCAUUID)))
                    {
